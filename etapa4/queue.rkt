...
(define (queue-empty? q)
  (and (equal? (queue-size-l q) 0) (equal? (queue-size-r q) 0)))


; TODO
; Implementați funcția rotate, conform axiomelor de mai sus.
; Atenție: ce tip trebuie să aibă Acc? -> flux
...
(define (enqueue x q)
  (if (equal? (queue-size-l q) (queue-size-r q))
      ; necesita o rotatie pe queue actualizat
      (struct-copy queue q [left (rotate (queue-left q) (cons x (queue-right q)) empty-stream)] [right '()] [size-r 0] [size-l (add1 (+ (queue-size-l q) (queue-size-r q)))])
      ; nu necesita rotatie, doar adauga in right
...
(define (dequeue q)
  (if (equal? (queue-size-l q) (queue-size-r q))
      ; necesita o rotatie pe queue actualizat
      (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)] [right '()] [size-r 0] [size-l (sub1 (+ (queue-size-l q) (queue-size-r q)))])
      ; nu necesita rotatie, doar scoate din left
      (struct-copy queue q [left (stream-rest (queue-left q))] [size-l (sub1 (queue-size-l q))])))
